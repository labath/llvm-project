...
#include "third_party/llvm/llvm-project/lldb/source/Plugins/SymbolFile/DWARF/DIERef.h"
#include "third_party/llvm/llvm-project/lldb/source/Plugins/SymbolFile/DWARF/NameToDIE.h"
...
void ManualDWARFIndex::Index() {
  ...
  auto for_each_unit = [&](auto &&fn, auto &&reduce) {
    ...
    auto wrapper = [&fn, &reduce, &next_unit_idx, &units_indexed,
                    &units_to_index, &progress, num_threads](size_t worker_id) {
...
#define FINALIZE_FN(index)([ this, &sets, &progress ] {                        \
  auto &result = this->m_set.index;                                            \
  for (auto &set : sets) result.Append(set.index);                             \
  progress.Increment();                                                        \
})
  task_group.async(FINALIZE_FN(function_basenames));
  task_group.async(FINALIZE_FN(function_fullnames));
  task_group.async(FINALIZE_FN(function_methods));
  task_group.async(FINALIZE_FN(function_selectors));
  task_group.async(FINALIZE_FN(objc_class_selectors));
  task_group.async(FINALIZE_FN(globals));
  task_group.async(FINALIZE_FN(types));
  task_group.async(FINALIZE_FN(namespaces));
  task_group.wait();
